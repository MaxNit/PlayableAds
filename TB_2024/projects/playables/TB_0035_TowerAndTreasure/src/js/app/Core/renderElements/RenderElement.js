...
export class RenderElement {
    ...
    findClosestContainer(containers) {
        const spriteBounds = this.node.getBounds(true);
    
        const spriteCenterX = spriteBounds.x + spriteBounds.width / 2;
        const spriteCenterY = spriteBounds.y + spriteBounds.height / 2;
    
        let closestContainer = null;
        let closestDistance = Number.MAX_SAFE_INTEGER;
    
        containers.forEach(container => {
            const containerBounds = container.node.getBounds(true);
            ...
            const distance = Math.sqrt(Math.pow(spriteCenterX - containerCenterX, 2) + Math.pow(spriteCenterY - containerCenterY, 2));
